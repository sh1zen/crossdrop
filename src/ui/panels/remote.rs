...
impl Component for RemotePanel {
    fn render(&mut self, f: &mut Frame, app: &App, area: Rect) {
        ...
        let title = format!(
            " Remote Files: {} @ {} ",
            get_display_name(app, app.state.remote.peer.as_deref().unwrap_or("?")),
            app.state.remote.path
        );

        let path_widget = Paragraph::new(app.state.remote.path.as_str()).block(
            Block::default()
                .title(" Remote Path ")
                .borders(Borders::ALL)
        ...
        let items: Vec<ListItem> = app
            ...
            .enumerate()
            .map(|(i, entry)| {
                let style = if i == app.state.remote.selected {
                    Style::default()
                        .fg(Color::Black)
                        .bg(Color::White)
...
impl Handler for RemotePanel {
    fn handle_key(&mut self, app: &mut App, node: &PeerNode, key: KeyCode) -> Option<Action> {
        match key {
            KeyCode::Esc => {
                app.state.remote.peer = None;
                app.state.remote.entries.clear();
                Some(Action::SwitchMode(Mode::Peers))
            }
            KeyCode::Up => {
                if !app.state.remote.entries.is_empty() {
                    if app.state.remote.selected == 0 {
                        app.state.remote.selected = app.state.remote.entries.len() - 1;
                    } else {
                        app.state.remote.selected -= 1;
                    }
                }
                Some(Action::None)
            }
            ...
            KeyCode::Enter => {
                if let Some(entry) = app.state.remote.entries.get(app.state.remote.selected).cloned() {
                    if entry.is_dir {
                        ...
                        app.state.remote.path = new_path.clone();
                        app.state.remote.entries.clear();
            ...
            KeyCode::Backspace => {
                ...
                if app.state.remote.path != "/" {
                    let path = app.state.remote.path.trim_end_matches('/');
                    if let Some(last_slash) = path.rfind('/') {
                        app.state.remote.path = if last_slash == 0 {
                            "/".to_string()
                        } else {
                    ...
                    if let Some(peer_id) = &app.state.remote.peer {
                        let peer_id = peer_id.clone();
                        let path = app.state.remote.path.clone();
                        let node = node.clone();
                        tokio::spawn(async move {
