...
pub struct FilesInfoPopup {
    ...
    /// For active transfers: shows state like "Interrupted", "Resumable", etc.
...
impl Component for FilesPanel {
    fn render(&mut self, f: &mut Frame, app: &App, area: Rect) {
        ...
        if has_active {
                ...
                .enumerate()
            {
                ...
                let is_selected = idx == selected_idx && app.state.files.focus_active;
                let marker = if is_selected { "▶ " } else { "  " };

                // State indicator for non-active transfers - prominent pending state
                let (state_prefix, state_color) = match txn.state {
                    TransactionState::Interrupted => ("⏸ PAUSED ", Color::Yellow),
                    TransactionState::Resumable => ("↻ AWAITING RESUME ", Color::Magenta),
                    TransactionState::Pending => ("⏳ PENDING ", Color::Yellow),
                    _ => ("", Color::Reset),
                };
        ...
        for (i, rec) in engine_history.iter().rev().enumerate() {
            ...
            let (arrow, color) = direction_style(rec.direction);
            ...
            let is_selected = i == history_selected && !app.state.files.focus_active;
            let marker = if is_selected { "▶ " } else { "  " };
            ...
            let status_color = rec.status.color();
        ...
        // Render info popup if active (use full area so popup can be centered)
    ...
    fn on_blur(&mut self, app: &mut App) {
        ...
        app.state.files.search.clear();
        app.state.files.focus_active = true;
...
impl FilesPanel {
    fn render_info_popup(&self, f: &mut Frame, popup: &FilesInfoPopup, area: Rect) {
        ...
        // Add state info for active transfers
        if let Some(ref state) = popup.active_state {
            let state_color = match state.as_str() {
                "Interrupted" => Color::Yellow,
                "Resumable" => Color::Magenta,
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    popup.status.label(),
                    Style::default().fg(popup.status.color()),
                ),
            ]));
        } else {
            // History entry - show timestamp and status
            lines.push(Line::from(vec![
        ...
        if popup.active_state.is_some() {
            lines.push(Line::from(vec![Span::styled(
                "Press Enter to close | Press 'x' to remove transfer",
                Style::default().fg(Color::DarkGray),
            )]));
        } else {
            lines.push(Line::from(vec![Span::styled(
...
impl Handler for FilesPanel {
    fn handle_key(&mut self, app: &mut App, _node: &PeerNode, key: KeyCode) -> Option<Action> {
        // If info popup is open, handle close or remove
        if let Some(ref popup) = self.info_popup {
            if popup.active_state.is_some() {
                // Active transfer popup - allow 'x' to remove
                return match key {
        ...
        match key {
            ...
            KeyCode::Tab => {
                ...
                app.state.files.focus_active = !app.state.files.focus_active;
                None
            }
            KeyCode::Char('/') => {
                // Enter search mode
                app.state.files.search_mode = true;
                app.state.files.search.clear();
                ...
                if app.state.files.focus_active {
                    ...
                    if active_count > 0 {
                        app.state.files.active_transfer_idx =
                            (app.state.files.active_transfer_idx + 1).min(active_count - 1);
                    }
                } else {
                    // Move selection in history
                    let history_len = app.engine.transfer_history().len();
                    if history_len > 0 {
                        app.state.files.history_selected_idx =
                            (app.state.files.history_selected_idx + 1).min(history_len - 1);
                    }
                }
                Some(Action::None)
            }
            KeyCode::Enter | KeyCode::Char(' ') => {
                if app.state.files.search_mode {
                    // Exit search mode on Enter (Space goes to char handler in search mode)
                ...
                if app.state.files.focus_active && has_active {
                    ...
                    if !active_txns.is_empty() {
                        ...
                        if let Some(txn) = active_txns.get(idx) {
                            let state_str = match txn.state {
                                ...
                                TransactionState::Interrupted => "Interrupted",
                                TransactionState::Resumable => "Resumable",
                                _ => "Unknown",
                            };
                            let status = match txn.state {
                                TransactionState::Interrupted => TransferStatus::Cancelled,
                                TransactionState::Resumable => TransferStatus::Cancelled,
                                TransactionState::Pending => TransferStatus::Ok,
                                _ => TransferStatus::Ok,
                            };
                            self.info_popup = Some(FilesInfoPopup {
                                direction: txn.direction,
                                display_name: txn.display_name.clone(),
                                peer_name: get_display_name(app, &txn.peer_id),
                                total_size: txn.total_size,
                                file_count: txn.total_file_count(),
                                timestamp: String::new(),
                                status,
                                active_state: Some(state_str.to_string()),
                            });
                        }
                    }
                    ...
                    if history_len > 0 {
                        let idx = app.state.files.history_selected_idx.min(history_len - 1);
            ...
            KeyCode::Char('x') | KeyCode::Char('X') => {
                ...
                // Cancel/remove the selected active transfer (including Interrupted/Resumable)
                if app.state.files.focus_active && has_active {
                    let active_ids: Vec<uuid::Uuid> = app
                        .engine
                        .transactions()
                    ...
                    if history_len > 0 {
                        ...
                        if app.engine.delete_history_entry(actual_idx) {
                            ...
                            if app.state.files.history_selected_idx >= new_len && new_len > 0 {
                                app.state.files.history_selected_idx = new_len - 1;
                            }
