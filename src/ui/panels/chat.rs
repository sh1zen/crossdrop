...
impl Component for ChatPanel {
    fn render(&mut self, f: &mut Frame, app: &App, area: Rect) {
        ...
        let sidebar_items: Vec<ListItem> = targets
            ...
            .enumerate()
            .map(|(i, target)| {
                let is_selected = i == app.state.chat.sidebar_idx;
        ...
        let target_label = app.state.chat.target.label(&app.state.peers.names);

        // Render messages from the logical message table
        let filtered: Vec<&Message> = app.state.chat.messages.messages_for(&app.state.chat.target);
        ...
        let title = match &app.state.chat.target {
            ChatTarget::Room => format!(" Room Chat ({} peers) ", app.state.peers.list.len()),
            ChatTarget::Peer(_) => format!(" DM with {} ", target_label),
        };

        let border_color = match &app.state.chat.target {
            ChatTarget::Room => Color::Yellow,
            ChatTarget::Peer(_) => accent,
        };
        ...
        let input_title = match &app.state.chat.target {
    ...
    fn on_focus(&mut self, app: &mut App, _node: &PeerNode) {
        ...
        match &app.state.chat.target {
            ChatTarget::Room => app.state.chat.unread.reset_room(),
            ChatTarget::Peer(pid) => app.state.chat.unread.reset_peer(&pid.clone()),
        }
    }
    ...
    fn handle_key(&mut self, app: &mut App, node: &PeerNode, key: KeyCode) -> Option<Action> {
        match key {
            ...
            KeyCode::Tab => {
                ...
                let targets = app.chat_targets();
                if !targets.is_empty() {
                    app.state.chat.sidebar_idx = (app.state.chat.sidebar_idx + 1) % targets.len();
                    let new_target = targets[app.state.chat.sidebar_idx].clone();
            ...
            KeyCode::BackTab => {
                ...
                let targets = app.chat_targets();
                if !targets.is_empty() {
                    if app.state.chat.sidebar_idx == 0 {
                        app.state.chat.sidebar_idx = targets.len() - 1;
                    } else {
                        app.state.chat.sidebar_idx -= 1;
                    }
                    let new_target = targets[app.state.chat.sidebar_idx].clone();
                    app.switch_chat_target(new_target);
                }
            ...
            KeyCode::Enter => {
                if !app.input.is_empty() {
                    ...
                    if let Some(cmd_result) = parse_command(&input) {
                        ...
                        match cmd_result {
                            Ok(ChatCommand::Clear) => {
                                let target_snap = match &app.state.chat.target {
                                    ChatTarget::Room => ChatTargetSnapshot::Room,
                                    ChatTarget::Peer(pid) => ChatTargetSnapshot::Peer(pid.clone()),
                                };
                                app.state.chat.messages.clear_target(&app.state.chat.target);
                            ...
                            Ok(ChatCommand::Help) => {
                                ...
                                let help_text = COMMAND_HELP
                                    ...
                                    .collect::<Vec<_>>()
                                    .join("\n");
                                app.state.chat.messages.insert(Message {
                                    id: Uuid::new_v4(),
                                    sender: MessageSender::Me,
                                    text: format!("Available commands:\n{}", help_text),
                                    timestamp: format_timestamp_now(),
                                    target: app.state.chat.target.clone(),
                                });
                    ...
                    let target = app.state.chat.target.clone();
...
fn send_typing_throttled(app: &mut App, node: &PeerNode) {
    let should_send = match app.state.chat.last_typing_sent {
    ...
    app.state.chat.last_typing_sent = Some(Instant::now());
