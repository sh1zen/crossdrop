...
impl Component for ConnectPanel {
    fn render(&mut self, f: &mut Frame, app: &App, area: Rect) {
        let constraints = if app.state.peers.connecting.is_empty() {
            vec![
                Constraint::Min(1),
                Constraint::Length(3),
                Constraint::Min(1),
            ...
            vec![
                ...
                Constraint::Length(2 + app.state.peers.connecting.len() as u16),
                Constraint::Min(1),
            ]
        };
        ...
        if !app.state.peers.connecting.is_empty() {
            let mut connecting_items = Vec::new();
            for (peer_id, status) in &app.state.peers.connecting {
