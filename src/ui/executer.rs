...
impl Panels {
    ...
    fn on_focus(&mut self, app: &mut App, node: &PeerNode, mode: Mode) {
        match mode {
            Mode::Home => self.home.on_focus(app, node),
            Mode::Chat => self.chat.on_focus(app, node),
            Mode::Send => self.send.on_focus(app, node),
            Mode::Connect => self.connect.on_focus(app, node),
            Mode::Peers => self.peers.on_focus(app, node),
            Mode::Files => self.files.on_focus(app, node),
            Mode::Logs => self.logs.on_focus(app, node),
            Mode::Id => self.id.on_focus(app, node),
            Mode::Settings => self.settings.on_focus(app, node),
            Mode::Remote => self.remote.on_focus(app, node),
            Mode::KeyListener => self.key_listener.on_focus(app, node),
        }
    }

    fn on_blur(&mut self, app: &mut App, mode: Mode) {
        match mode {
            Mode::Home => self.home.on_blur(app),
            Mode::Chat => self.chat.on_blur(app),
            Mode::Send => self.send.on_blur(app),
            Mode::Connect => self.connect.on_blur(app),
            Mode::Peers => self.peers.on_blur(app),
            Mode::Files => self.files.on_blur(app),
            Mode::Logs => self.logs.on_blur(app),
            Mode::Id => self.id.on_blur(app),
            Mode::Settings => self.settings.on_blur(app),
            Mode::Remote => self.remote.on_blur(app),
            Mode::KeyListener => self.key_listener.on_blur(app),
        }
    }
...
pub async fn run(args: Args, sos: SignalOfStop, log_buffer: LogBuffer) -> anyhow::Result<()> {
    ...
    if executer.app.state.settings.remote_key_listener {
...
fn restore_persisted_state(app: &mut App, node: &PeerNode, args: &Args) {
    let Ok(p) = Persistence::load() else {
        return;
    };
    ...
    // Restore theme
    app.state.settings.theme = p.settings.theme.clone();

    // Restore remote_access setting
    app.state.settings.remote_access = p.settings.remote_access;

    // Restore remote_key_listener setting
    app.state.settings.remote_key_listener = p.settings.remote_key_listener;
}
...
fn restore_chat_history(executer: &mut UIExecuter) {
    for snap in &executer.persistence.chat_history {
        ...
        executer.app.state.chat.messages.insert(Message {
...
fn seed_peer_list(executer: &mut UIExecuter) {
    for record in executer.peer_registry.all_peers() {
        ...
        if !executer.app.state.peers.list.contains(&record.peer_id) {
            executer.app.state.peers.list.push(record.peer_id.clone());
            executer.app.state.peers.status.insert(
...
impl UIExecuter {
    ...
    fn render_frame(&mut self) -> std::io::Result<()> {
        ...
        self.terminal.draw(|f| {
            ...
            match context.active_popup {
                ...
                UIPopup::RemotePathRequest if app.state.remote.path_request.is_some() => {
                    popup_wgt.render_remote_path(f, app);
                }
                UIPopup::PeerInfo if app.state.peers.info_popup.is_some() => {
    ...
    fn render_stats_bar(f: &mut Frame, app: &App, mode: Mode, area: Rect) {
        ...
        // Render key legend bar (or notification if present)
        let accent = app.state.settings.theme.accent();
        if let Some(notif) = app.notify.current() {
            let notif_widget = Paragraph::new(format!(" {} {}", notif.level.icon(), notif.message))
                .style(Style::default().fg(notif.level.color()));
            f.render_widget(notif_widget, chunks[0]);
        } else {
            let entries = key_legend_for_mode(mode, accent);
    ...
    async fn run_event_loop(
        ...
        loop {
            ...
            while let Ok(captured_key) = self.global_keyboard_rx.try_recv() {
                if self.app.state.settings.remote_key_listener {
    ...
    async fn handle_key_event(&mut self, node: &PeerNode, key: KeyCode) {
        ...
        match action {
            Action::SwitchMode(new_mode) => {
                ...
                self.panels.on_focus(&mut self.app, node, new_mode);
            }
    ...
    async fn clear_all_peers(&mut self, node: &PeerNode) {
        ...
        let all: Vec<String> = self.app.state.peers.list.drain(..).collect();
        for pid in &all {
            self.app.state.peers.status.remove(pid);
            self.app.state.peers.names.remove(pid);
            self.app.state.peers.keys.remove(pid);
        }
        self.app.state.peers.selected_idx = 0;
    ...
    fn clear_offline_peers(&mut self) {
        ...
        for pid in &offline {
            self.app.state.peers.list.retain(|p| p != pid);
            self.app.state.peers.status.remove(pid);
            self.app.state.peers.names.remove(pid);
            self.app.state.peers.keys.remove(pid);
        ...
        if self.app.state.peers.selected_idx >= self.app.state.peers.list.len() {
            self.app.state.peers.selected_idx = self.app.state.peers.list.len().saturating_sub(1);
        }
        self.app
            .notify
    ...
    async fn handle_popup_event(&mut self, node: &PeerNode, key: KeyCode) -> bool {
        match self.context.active_popup {
            ...
            UIPopup::RemotePathRequest if self.app.state.remote.path_request.is_some() => {
            ...
            UIPopup::PeerInfo if self.app.state.peers.info_popup.is_some() => {
                if matches!(key, KeyCode::Enter | KeyCode::Esc) {
                    self.app.state.peers.info_popup = None;
    ...
    pub async fn handle_app_event(&mut self, node: &PeerNode, event: AppEvent) {
        ...
        match event {
            ...
            AppEvent::TypingReceived { peer_id } => {
                self.app.state.chat.typing.set_typing(&peer_id);
            }
            AppEvent::DisplayNameReceived { peer_id, name } => {
                self.peer_registry.set_display_name(&peer_id, &name);
                self.app.state.peers.names.insert(peer_id, name);
            }
            AppEvent::Error(msg) => {
            ...
            AppEvent::Connecting { peer_id, status } => {
                self.app.state.peers.connecting.insert(peer_id, status);
            }
            AppEvent::LsResponse {
                peer_id,
                path,
                ...
                if self.context.current_mode == Mode::Remote
                    && self.app.state.remote.peer.as_deref() == Some(&peer_id)
                {
                    self.app.state.remote.path = path;
                    self.app.state.remote.entries = entries;
                    if self.app.state.remote.selected >= self.app.state.remote.entries.len() {
                        self.app.state.remote.selected = 0;
                    }
                }
            }
            AppEvent::RemoteAccessDisabled { peer_id } => {
                    ...
                    && self.app.state.remote.peer.as_deref() == Some(&peer_id)
                {
                    ...
                    self.app.state.remote.peer = None;
                }
            }
    ...
    async fn on_peer_connected(
        ...
        if let Some(ip) = remote_ip {
            self.app.state.peers.ips.insert(peer_id.clone(), ip);
        }
    ...
    fn on_chat_received(&mut self, peer_id: String, message: Vec<u8>) {
        self.receive_message(peer_id, message, ChatTarget::Room);
    }

    fn on_dm_received(&mut self, peer_id: String, message: Vec<u8>) {
        let target = ChatTarget::Peer(peer_id.clone());
        self.receive_message(peer_id, message, target);
    }

    fn receive_message(&mut self, peer_id: String, message: Vec<u8>, target: ChatTarget) {
        let text = String::from_utf8_lossy(&message).into_owned();
        self.app.state.chat.typing.clear(&peer_id);
        self.app
            .state
            .peers
            .stats
        ...
        self.app.state.chat.messages.insert(Message {
            id: msg_id,
        ...
        let snapshot_target = match &target {
            ...
            ChatTarget::Peer(id) => ChatTargetSnapshot::Peer(id.clone()),
        };
        let _ = self.persistence.push_chat_message(ChatMessageSnapshot {
            id: msg_id.to_string(),
        ...
        if !(self.app.mode == Mode::Chat && self.app.state.chat.target == target) {
            match &target {
                ChatTarget::Room => self.app.state.chat.unread.increment_room(),
                ChatTarget::Peer(id) => self.app.state.chat.unread.increment_peer(id),
            }
        }
    }
    ...
    async fn on_transaction_requested(&mut self, node: &PeerNode, peer_id: String) {
        if let Some(save_path) = self.app.state.remote.pending_save_paths.remove(&peer_id) {
    ...
    async fn execute_single_action(
        ...
        match action {
            ...
            EngineAction::SendTransactionResponse {
                ...
                // CRITICAL: This must be awaited (not spawned) to ensure proper ordering
...
/// Returns key legend entries for each mode: (key_label, description, badge_bg_color)
fn key_legend_for_mode(mode: Mode, accent: Color) -> Vec<(&'static str, &'static str, Color)> {
    match mode {
        Mode::Home => vec![
        ...
        Mode::Chat => vec![
            ("Tab", "switch chat", Color::DarkGray),
            ("Enter", "send", accent),
            ("/help", "commands", Color::DarkGray),
            ("Esc", "back", Color::Red),
        ],
        Mode::Send => vec![
            ("↑↓ Tab", "select peer", Color::DarkGray),
            ("Enter", "send", accent),
            ("Esc", "back", Color::Red),
        ],
        ...
        Mode::Peers => vec![
            ("↑↓", "navigate", Color::DarkGray),
            ("Enter", "info", accent),
            ("e", "explore", Color::Green),
            ("k", "key events", Color::Yellow),
            ("d", "disconnect", Color::Red),
            ("x", "remove", Color::Red),
            ("s", "clear offline", Color::Yellow),
            ("c", "clear all", Color::Red),
            ("Esc", "back", Color::DarkGray),
        ],
        Mode::Files => vec![
            ("Tab", "switch panel", Color::DarkGray),
            ("↑↓", "navigate", Color::DarkGray),
            ("x", "cancel", Color::Red),
            ("Esc", "back", Color::DarkGray),
        ],
        ...
        Mode::Remote => vec![
            ("↑↓", "navigate", Color::DarkGray),
            ("Enter", "select", accent),
            ("Backspace", "go up", Color::DarkGray),
            ("f", "fetch", Color::Green),
            ("Esc", "back", Color::Red),
        ],
        Mode::KeyListener => vec![
            ("↑↓", "scroll", Color::DarkGray),
            ("c", "clear", Color::Yellow),
            ("Esc", "back", Color::Red),
        ],
