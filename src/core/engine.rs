...
pub enum EngineAction {
    ...
    CancelTransaction {
        peer_id: String,
        transaction_id: Uuid,
        /// File IDs belonging to this transaction — used to clean up in-flight
        /// receiver state (recv_state, pending_chunks, accepted_destinations).
        file_ids: Vec<Uuid>,
    },
    /// Clean up receiver-side file state after a cancel received from the remote
...
impl TransferEngine {
    ...
    fn register_replay_guard(&mut self, transaction_id: Uuid) {
        self.replay_guard.register_transaction(transaction_id);
    }
    ...
    pub fn accept_incoming(&mut self, dest_path: String) -> Result<EngineOutcome> {
        ...
        // Compute and persist the authoritative full path for each file.
        // accepted_destinations stores the FULL per-file path (not a base dir),
        // so handle_metadata can use it directly without any secondary join.
        let dest_pb = PathBuf::from(&dest_path);
        for fid in &txn.file_order {
            if let Some(tf) = txn.files.get_mut(fid) {
                let file_dest = Transaction::resolve_file_dest(
                    &dest_pb,
                    txn.parent_dir.as_deref(),
                    &tf.relative_path,
                );
                tf.full_path = Some(file_dest.to_string_lossy().into_owned());
            }
        }

        // Register the complete per-file destination paths with the transport layer.
        // PrepareReceive stores file_id → full_path; handle_metadata uses it directly.
        //
        ...
        let files: Vec<(Uuid, PathBuf)> = txn
            ...
            .iter()
            .filter_map(|fid| {
                let tf = txn.files.get(fid)?;
                let full_path = if let Some(ref fp) = tf.full_path {
                    PathBuf::from(fp)
                } else {
                    Transaction::resolve_file_dest(
                        &dest_pb,
                        txn.parent_dir.as_deref(),
                        &tf.relative_path,
                    )
    ...
    pub fn process_event(&mut self, event: &AppEvent) -> EngineOutcome {
        match event {
            ...
            AppEvent::AckReceived { context } => match context {
            ...
            AppEvent::TransactionCancelled {
                ...
                // Collect peer_id and file_ids before archiving the transaction.
                let (peer_id, file_ids) =
                    if let Some(txn) = self.transactions.get_active_mut(transaction_id) {
                        let p = txn.peer_id.clone();
                        let f = txn.file_order.clone();
                        txn.cancel();
                        (p, f)
                    } else {
                        (String::new(), Vec::new())
    ...
    fn handle_peer_disconnected(&mut self, peer_id: &str) -> EngineOutcome {
        ...
        if !txn_ids.is_empty() {
            // Guard against load failure: do not fall back to Default, which would
            // save a blank state and destroy all other persisted data.
            match Persistence::load() {
                ...
                Err(e) => error!(
                    event = "persistence_load_failure_on_disconnect",
                    error = %e,
                    "Cannot checkpoint interrupted transactions: persistence load failed"
                ),
            }
        }
    ...
    fn handle_transaction_requested(
        ...
        // Debug: log manifest file_ids
        let manifest_file_ids: Vec<Uuid> = manifest.files.iter().map(|f| f.file_id).collect();
        info!(
            event = "manifest_received",
            %transaction_id,
            %display_name,
            file_count = manifest.files.len(),
            file_ids = ?manifest_file_ids,
            parent_dir = ?manifest.parent_dir,
            "Received transaction request with manifest"
        );
    ...
    fn handle_transaction_accepted(
        ...
        // Debug: log the file_ids in the outbound transaction
        let outbound_file_ids: Vec<Uuid> = txn.file_order.clone();
        info!(
            event = "transfer_peer_accepted",
            %transaction_id,
            display_name = %txn.display_name,
            file_count = outbound_file_ids.len(),
            file_ids = ?outbound_file_ids,
            parent_dir = ?txn.parent_dir,
            "Outbound transaction accepted - will send these file_ids"
        );
        txn.activate();
    ...
    pub fn handle_peer_reconnected(&mut self, peer_id: &str) -> EngineOutcome {
        ...
        if !active_ids.is_empty() {
            ...
            match Persistence::load() {
                ...
                Err(e) => error!(
                    event = "persistence_load_failure_on_reconnect",
                    error = %e,
                    "Cannot checkpoint zombie transactions: persistence load failed"
                ),
            }
        }
    ...
    ///
    /// Returns the complete authoritative destination path per file — identical
    ...
    fn incomplete_file_destinations(txn: &Transaction) -> Vec<(Uuid, PathBuf)> {
        txn.file_order
            ...
            .filter_map(|fid| {
                ...
                // Primary: use the pre-computed full_path (set by accept_incoming).
                let full_path = if let Some(ref fp) = f.full_path {
                    PathBuf::from(fp)
                } else if let Some(dest) = txn.dest_path.as_deref() {
                    // Fallback: recompute from dest_path (e.g. resume after crash before persist).
                    Transaction::resolve_file_dest(
                        dest,
                        txn.parent_dir.as_deref(),
                        &f.relative_path,
    ...
    fn persist_active_transaction(&self, txn_id: &Uuid) {
        ...
        let src = self.source_paths.get(txn_id).map(String::as_str);
        let snapshot = txn.to_snapshot_with_source(src);
        // Guard against load failure: saving a default Persistence would overwrite
        // all existing state (history, settings, other transactions) with a blank file.
        let mut p = match Persistence::load() {
            Ok(p) => p,
            Err(e) => {
                warn!(
                    event = "persist_active_load_failure",
                    transaction_id = %txn_id,
                    error = %e,
                    "Skipping transaction checkpoint: could not load persistence state"
                );
                return;
            }
        };
    ...
    fn archive_transaction_with_status(&mut self, txn_id: Uuid, status: TransferStatus) {
        // Build the history record before archiving (borrows active map).
        if let Some(txn) = self.transactions.active.get(&txn_id) {
            ...
            self.transfer_history.push(record.clone());

            // Persist: append history record AND remove the active transaction in
            // one atomic save. The previous two-step pattern (push_transfer_record
            // saves, then remove_transaction saves) left a window where a crash
            // between the two saves would leave the transaction in both the history
            // and the active-transactions table — causing it to be incorrectly
            // offered for resume on restart.
            let mut p = match Persistence::load() {
                Ok(p) => p,
                Err(e) => {
                    warn!(
                        event = "persistence_load_failure_on_archive",
                        transaction_id = %txn_id,
                        error = %e
                    );
                    Persistence::default()
            ...
            p.transfer_history.push(record);
            p.transactions.remove(&txn_id);
            if let Err(e) = p.save() {
                warn!(event = "history_persist_failure", error = %e);
            }
        } else {
            // Transaction not in active map — just remove any stale snapshot.
            if let Ok(mut p) = Persistence::load() {
                let _ = p.remove_transaction(&txn_id);
            }
        }
