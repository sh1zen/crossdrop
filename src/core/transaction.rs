...
impl TransactionState {
    ...
    /// Returns true if the transfer is in an active, pending, or interruptible state.
    ///
    /// Equivalent to `!is_terminal()`. Used by the UI to identify transfers to
    /// display in the Active Transfers section.
    #[inline]
...
impl TransactionFile {
    pub fn new(file_id: Uuid, relative_path: String, filesize: u64) -> Self {
        let total_chunks = helpers::compute_total_chunks(filesize);
        Self {
            file_id,
            relative_path,
            full_path: None,
...
impl Transaction {
    ...
    fn build_file_map_inbound(
        ...
        for entry in &manifest.files {
            // Skip duplicate file_ids to prevent incorrect file counts
            if map.contains_key(&entry.file_id) {
                continue;
            }
            let mut tf =
    ...
    /// Compute the **base directory** into which received files are written.
    ///
    /// This is the single authoritative implementation of the path formula:
    ///   `dest_path / parent_dir`   (folder transfer)
    ///   `dest_path`                (single-file transfer)
    ///
    /// Every code path that registers file destinations (`PrepareReceive`) or
    /// computes where files will be saved **must** call this function to
    /// guarantee identical behaviour between a fresh transfer and a resume.
    pub fn resolve_base_dir(dest_path: &std::path::Path, parent_dir: Option<&str>) -> PathBuf {
        match parent_dir {
            Some(dir) => dest_path.join(dir),
            None => dest_path.to_path_buf(),
        }
    }

    /// Compute the **full destination path** for a single file.
    ///
    /// Equivalent to `resolve_base_dir(dest, parent) / sanitize_relative_path(relative_path)`.
    /// Sanitizes the relative path to prevent directory traversal and other attacks.
    pub fn resolve_file_dest(
        dest_path: &std::path::Path,
        parent_dir: Option<&str>,
        relative_path: &str,
    ) -> PathBuf {
        let sanitized = helpers::sanitize_relative_path(relative_path);
        Self::resolve_base_dir(dest_path, parent_dir).join(sanitized)
    }
