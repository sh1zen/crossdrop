...
///
/// When both peers auto-reconnect simultaneously, a race can leave the
/// `peers` map pointing at a *different* `WebRTCConnection` than the one
/// whose data-channel handlers actually receive traffic.  By sharing these
/// Arcs across every connection for the same peer, `PrepareReceive`
/// registrations are visible to whichever handler ends up processing the
/// sender's Metadata frames.
pub struct SharedReceiverState {
    pub accepted_destinations: Arc<RwLock<HashMap<Uuid, std::path::PathBuf>>>,
...
impl ConnectionState {
    ...
    ///
    /// If `shared` is provided (from an earlier connection for the same peer),
    /// the new connection reuses the same `accepted_destinations` and
    /// `resume_bitmaps` Arcs so that file registrations are never lost during
    /// a connection replacement race.
    fn new(shared: Option<SharedReceiverState>) -> Self {
        let shared = shared.unwrap_or_else(|| SharedReceiverState {
        ...
        Self {
            ...
            accepted_destinations: shared.accepted_destinations,
            resume_bitmaps: shared.resume_bitmaps,
...
fn expect_sdp(msg: SignalingMessage, expected: &'static str) -> Result<RTCSessionDescription> {
    let sdp = match msg {
        SignalingMessage::Offer(s) if expected == "Offer" => s,
        SignalingMessage::Answer(s) if expected == "Answer" => s,
...
impl WebRTCConnection {
    ...
    async fn setup_peer(
        ...
        role: &'static str,
        remote_access_enabled: bool,
        shared_rx_state: Option<SharedReceiverState>,
    ) -> Result<(
        Arc<RTCPeerConnection>,
        ConnectionState,
        HandlerContext,
        Option<SessionKeyManager>,
    )> {
        let api = Self::create_webrtc_api().await?;
        let pc = Self::new_peer_connection(&api).await?;
        setup_connection_state_monitor(&pc, app_tx, role);
        let state = ConnectionState::new(shared_rx_state);
    ...
    pub async fn create_offer(
        ...
        let (pc, state, ctx, key_manager) = Self::setup_peer(
            &app_tx,
            &shared_key,
            key_manager,
            &wire_tx,
