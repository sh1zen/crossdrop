...
/// in the configured data directory so that multiple instances on the same
/// machine each get their own identity.
///
...
/// Generate iterator over secret key slot names (unbounded).
fn secret_key_slots() -> impl Iterator<Item = String> {
    std::iter::once("secret_key".to_string()).chain((1..).map(|i| format!("secret_key_{i}")))
}
