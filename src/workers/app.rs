...
/// Transfer/Send panel UI state.
pub struct TransferState {
    /// File/folder path input for sending.
...
/// Aggregated application state combining all sub-states.
pub struct State {
    ...
    /// Key Listener panel UI state.
    pub key_listener: KeyListenerState,
}
...
impl State {
    pub fn new(display_name: Option<String>) -> Self {
        Self {
            ...
            key_listener: KeyListenerState::new(),
        }
    }
}
...
impl App {
    ...
    pub fn selected_peer(&self) -> Option<&String> {
        self.state.peers.list.get(self.state.peers.selected_idx)
    }
    ...
    pub fn set_peer_offline(&mut self, peer_id: &str) {
        ...
        self.state.chat.typing.remove_peer(peer_id);

        // If we're viewing this peer's remote filesystem, exit remote mode
        if self.mode == Mode::Remote && self.state.remote.peer.as_deref() == Some(peer_id) {
            self.mode = Mode::Peers;
            self.state.remote.peer = None;
        }
    }
    ...
    pub fn remove_peer(&mut self, peer_id: &str) {
        ...
        if self.state.chat.target == ChatTarget::Peer(peer_id.to_string()) {
            self.state.chat.target = ChatTarget::Room;
            self.state.chat.sidebar_idx = 0;
        }
        // Clean up all peer-related data
        self.state.peers.names.remove(peer_id);
        self.state.peers.keys.remove(peer_id);
        self.state.chat.typing.remove_peer(peer_id);
        self.state.chat.unread.remove_peer(peer_id);
    ...
    pub fn is_peer_online(&self, peer_id: &str) -> bool {
        self.state.peers.status.get(peer_id).copied() == Some(PeerStatus::Online)
    }
    ...
    pub fn switch_chat_target(&mut self, target: ChatTarget) {
        self.state.chat.target = target.clone();
        match &target {
            ChatTarget::Room => self.state.chat.unread.reset_room(),
            ChatTarget::Peer(pid) => self.state.chat.unread.reset_peer(pid),
        }
    }

    /// Count unread messages in the Room chat.
    pub fn unread_room_count(&self) -> usize {
        self.state.chat.unread.room_count()
    }

    /// Count unread DM messages for a specific peer.
    pub fn unread_dm_count(&self, peer_id: &str) -> usize {
        self.state.chat.unread.peer_count(peer_id)
    }

    /// Total unread messages across all chat channels.
    pub fn total_unread(&self) -> usize {
        self.state.chat.unread.total()
    }
}
